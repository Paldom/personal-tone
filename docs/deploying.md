...
- Consumers run `npx skills add Paldom/personal-tone` — the CLI
  ...
  users can also `gh skill install Paldom/personal-tone <skill>
...
gh repo edit Paldom/personal-tone --visibility public --accept-visibility-change-consequences
...
gh api repos/Paldom/personal-tone/private-vulnerability-reporting --method PUT

# 3. Cut a versioned release (matches .claude-plugin/plugin.json) + topics
gh skill publish --tag v0.1.0
gh repo edit Paldom/personal-tone --add-topic skills-sh
#    (gh < 2.90 has no `gh skill`; equivalent fallback:
#     gh release create v0.1.0 --generate-notes
#     gh repo edit Paldom/personal-tone --add-topic agent-skills --add-topic skills-sh)
...
npx skills@latest add Paldom/personal-tone --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
env -u DISABLE_TELEMETRY -u DO_NOT_TRACK npx skills@latest add Paldom/personal-tone --skill '*' -a claude-code -y
...
curl -sIL https://skills.sh/Paldom/personal-tone | head -1
```
...
(`https://skills.sh/b/Paldom/personal-tone`) linking to the repo's
skills.sh page.
